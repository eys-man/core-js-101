...
function encodeToRot13(str) {
  ...
  const lo = 'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
  let res = '';
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    index = up.indexOf(str[i]);
    if( index > -1 ) char = up[index + 13];
    else char = str[i];

    index = lo.indexOf(str[i]);
    if( index > -1) char = lo[index + 13];
    else char = str[i];
